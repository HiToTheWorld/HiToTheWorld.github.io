function start() {
    ...
    if (answer != '' && answer != null && !(answer.match("☒"))) {
        ...
        if (firstPattern) {
            if (answer % firstPattern.length != 0) {
                for (let i = 0; i < answer % firstPattern.length; i++) {
                    answer += "☒"
                }
            }

            for (let rotate = 0; rotate < answer.length - 1; rotate++) {
                let artificialAnswer = lastArtAnswer.substring(1, lastArtAnswer.length)
